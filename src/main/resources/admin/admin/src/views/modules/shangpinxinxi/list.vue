<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"90%","margin":"30px auto 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20220726/bd28bed57c07435ab63ec9a9b5eb4c5c.png) no-repeat center top / 100% 100%","width":"auto","fontSize":"14px","height":"44px"}' v-if="isAuth('shangpinxinxi','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20220726/bd28bed57c07435ab63ec9a9b5eb4c5c.png) no-repeat center top / 100% 100%","width":"auto","fontSize":"14px","height":"44px"}' v-if="isAuth('shangpinxinxi','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"5px solid #3FA8C4","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(63, 168, 196, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('shangpinxinxi','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"5px solid #3FA8C4","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(63, 168, 196, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('shangpinxinxi','咨询反馈')" type="success" size="mini" @click="zixunfankuiCrossAddOrUpdateHandler(scope.row,'cross','','','')">咨询反馈</el-button>
							<el-button :style='{"border":"5px solid #3FA8C4","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(63, 168, 196, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('shangpinxinxi','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>


							<el-button :style='{"border":"5px solid #3FA8C4","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(63, 168, 196, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('shangpinxinxi','查看评论')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">查看评论</el-button>



							<el-button :style='{"border":"5px solid #3FA8C4","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(63, 168, 196, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('shangpinxinxi','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<zixunfankui-cross-add-or-update v-if="zixunfankuiCrossAddOrUpdateFlag" :parent="this" ref="zixunfankuiCrossaddOrUpdate"></zixunfankui-cross-add-or-update>
...
import zixunfankuiCrossAddOrUpdate from "../zixunfankui/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      zixunfankuiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    zixunfankuiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.zixunfankuiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.zixunfankuiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.zixunfankuiCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          shangpinpinpai: row.shangpinpinpai,
